...
def main_dashboard():
    ...
    with tab2:
        ...
        company_cols = ['Company_Name', 'NASDAQ_Symbol', 'Last_Sale', 'Market_Cap', 'Lead_Product', 'Partnerships', 'Technology']
        df_companies = df_filtered[company_cols].drop_duplicates(subset=['Company_Name']).head(20)
        df_companies.columns = ['Company', 'Symbol', 'Stock Price', 'Market Cap', 'Lead Product', 'Partnerships', 'Technology']
        st.dataframe(df_companies, width='stretch', hide_index=True, use_container_width=True)
        
        st.markdown("<hr style='margin: 1.5rem 0;'>", unsafe_allow_html=True)
        
        # Clinical Trials Table
        st.markdown("**Clinical Trials Portfolio**")
        trial_cols = ['Company_Name', 'Trial_NCT_ID', 'Trial_Clinical_Phase', 'Trial_Overall_Status', 'Disease', 'Disease_Area', 'Investor_Highlights']
        df_trials = df_filtered[trial_cols].drop_duplicates(subset=['Trial_NCT_ID']).head(20)
        df_trials.columns = ['Company', 'Trial ID', 'Phase', 'Status', 'Disease', 'Disease Area', 'Highlights']
        st.dataframe(df_trials, width='stretch', hide_index=True, use_container_width=True)
        
        st.markdown("<hr style='margin: 1.5rem 0;'>", unsafe_allow_html=True)
        
        # Phase Distribution Table
        st.markdown("**Phase Distribution Analysis**")
        phase_dist = df_filtered['Trial_Clinical_Phase'].value_counts().reset_index()
        phase_dist.columns = ['Phase', 'Count']
        phase_dist['Percentage'] = (phase_dist['Count'] / phase_dist['Count'].sum() * 100).round(1).astype(str) + '%'
        st.dataframe(phase_dist, width='stretch', hide_index=True, use_container_width=True)
        
        st.markdown("<hr style='margin: 1.5rem 0;'>", unsafe_allow_html=True)
        
        # Trial Status Distribution Table
        st.markdown("**Trial Status Distribution**")
        status_dist = df_filtered['Trial_Overall_Status'].value_counts().reset_index()
        status_dist.columns = ['Status', 'Count']
        status_dist['Percentage'] = (status_dist['Count'] / status_dist['Count'].sum() * 100).round(1).astype(str) + '%'
        st.dataframe(status_dist, width='stretch', hide_index=True, use_container_width=True)
        
        st.markdown("<hr style='margin: 1.5rem 0;'>", unsafe_allow_html=True)
        
        # Disease Area Distribution Table
        st.markdown("**Disease Area Distribution**")
        disease_dist = df_filtered['Disease_Area'].value_counts().reset_index()
        disease_dist.columns = ['Disease Area', 'Count']
        disease_dist['Percentage'] = (disease_dist['Count'] / disease_dist['Count'].sum() * 100).round(1).astype(str) + '%'
        st.dataframe(disease_dist, width='stretch', hide_index=True, use_container_width=True)
        
        st.markdown("<hr style='margin: 1.5rem 0;'>", unsafe_allow_html=True)
        
        # Company Trial Count Table
        st.markdown("**Company Trial Count**")
        company_trial_count = df_filtered.groupby('Company_Name')['Trial_NCT_ID'].nunique().reset_index()
        company_trial_count.columns = ['Company', 'Trial Count']
        company_trial_count = company_trial_count.sort_values('Trial Count', ascending=False).head(20)
        st.dataframe(company_trial_count, width='stretch', hide_index=True, use_container_width=True)
        
        st.markdown("<hr style='margin: 1.5rem 0;'>", unsafe_allow_html=True)
        
        # Partnership Activity Table
        st.markdown("**Partnership Activity**")
        partnership_data = df_filtered[['Company_Name', 'Partnerships']].drop_duplicates(subset=['Company_Name'])
        partnership_data = partnership_data[partnership_data['Partnerships'].notna()].head(20)
        partnership_data.columns = ['Company', 'Partnerships']
        st.dataframe(partnership_data, width='stretch', hide_index=True, use_container_width=True)
        
        st.markdown("<hr style='margin: 1.5rem 0;'>", unsafe_allow_html=True)
        
        # Technology Diversity Table
        st.markdown("**Technology Platforms**")
        tech_data = df_filtered[['Company_Name', 'Technology']].drop_duplicates(subset=['Company_Name'])
        tech_data = tech_data[tech_data['Technology'].notna()].head(20)
        tech_data.columns = ['Company', 'Technology']
        st.dataframe(tech_data, width='stretch', hide_index=True, use_container_width=True)
